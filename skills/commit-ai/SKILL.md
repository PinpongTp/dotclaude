...
**`<MODEL_NAME>` is dynamic** — substitute the assistant's currently running model at commit time (e.g. `Claude Opus 4.7 (1M context)`, `Claude Sonnet 4.6`, `Claude Haiku 4.5`). Read it from the runtime environment; never hardcode.
...
Resolve `<MODEL_NAME>` to the actual running model before executing the `git commit`. Do not commit the literal placeholder.
